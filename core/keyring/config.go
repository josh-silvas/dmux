...
func (s *Settings) KeyFromSection(section, key, def string) (*ini.Key, error) {
	...
	if err != nil {
		pValue := promptui.Prompt{
			Label:   fmt.Sprintf("Enter `%s` %s value", section, key),
			Default: def,
		}
		def, err = pValue.Run()
		if err != nil {
			logrus.Fatalf("[%s]%s.Prompt(%s)", section, key, err)
		}
		value, err = sec.NewKey(key, def)
		if err != nil {
			return nil, err
		}
