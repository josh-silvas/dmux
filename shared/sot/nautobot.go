...
func (n Nautobot) consolePort(id string) (nautobot.ConsolePort, error) {
	p, err := n.ConsoleConnections(&url.Values{"device_id": []string{id}})
	if err != nil {
		return nautobot.ConsolePort{}, fmt.Errorf("cfg.Nautobot.ConsoleConnections:%w", err)
	}
	if len(p) == 0 {
...
func (n Nautobot) nbDeviceToSotDevice(d nautobot.Device) Device {
	ret := Device{
		Hostname: d.Name,
		IP: func() string {
